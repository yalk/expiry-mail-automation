...
for my $sheet (@{$excel -> {Worksheet}})
{
                                                    	...
                                                    	#loop starting from min row to max row
        {
                                                    	...
                                                    	#loop starting from min col to max col
            {
                ...
                if ($cell)
                {
					...
					if ($regexCell =~ /created/gi)
							#regex to find creation date
					{
						#storing the row and col index
						$createdRow=$row;
						$createdCol=$col;
					}
					if ($regexCell =~ /duration/gi)	
							#regex to find duration of service
					{
						#storing the row and col index
						$durationRow=$row;
						$durationCol=$col;
						last MOVETONEXT;
							#break loop and move on to next step
					}
				}
			}
		}
	...
	foreach my $row (1 .. $sheet -> {MaxRow})
	{
		...
		if ($cell)
		{
			...
			my $diff=$serviceEndDay-$todayDay;
							#calculate difference between end day and today
			...
			if($diff<=7)			#if number of days for service to get over is less than 7 then add $emailID to array and print
			{
				...
				{
					push @listOfEmailIDs,$emailID->{Val};
							#push email address in the array
				}
				else
				{
	...
	if($sendEmailInput=~ /^yes$/i)			#if input matches exactly with yes (i for case insensitive)
	{
		...
		foreach my $emailid(@listOfEmailIDs)											
		{
			...
			$message->Subject($emailSubject);
							#paste variable content in member of object
			$message->Body($emailBody);	
							#paste variable content in member of object
			#$message->Attach(@lots_of_files);
							#edit this if required
			#$message->Attach(@more_files); 
							#attachments are appended
			#$message->Attach($one_file);   
							#so multiple calls are allowed
			$message->send;																
			print "$emailid\n";															
